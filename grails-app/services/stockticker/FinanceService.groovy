...
class FinanceService {
	...
	def StockResult getStockResultScraped(String stockSymbol) {
		...
		GPathResult htmlParser;
		try {
			htmlParser = slurper.parse("http://uk.advfn.com/p.php?pid=qkquote&btn=&epic=$stockSymbol&symbol=")
		} catch (Exception ex) {
			log.error("Exception reached trying to parse stock quote - url: " + "http://uk.advfn.com/p.php?pid=qkquote&btn=&epic=$stockSymbol&symbol=", ex)
